...
def main():
    ...
    # `scscl` may be the module (with class `scscl`) or the class itself depending
    # on how scservo_sdk.__init__ exported symbols. Handle both cases.
    if hasattr(scscl, "scscl"):
        ScsclClass = scscl.scscl
    else:
        ScsclClass = scscl
    packet = ScsclClass(port)
