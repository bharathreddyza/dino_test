...
class RLWalk:
    def __init__(
        ...
        # Choose HWI implementation at runtime based on duck_config
        HWI_class = None
        if getattr(self.duck_config, "use_waveshare", False):
            try:
        ...
        if HWI_class is None:
            raise RuntimeError("No HWI implementation available (rustypot or waveshare)")

        self.hwi = HWI_class(self.duck_config, serial_port)
